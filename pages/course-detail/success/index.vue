            ...
            <div class="breadcrumb__content__wraper" data-aos="fade-up">
              <div class="breadcrumb__title" v-if="stores.last_course">
                <span style="font-size: 24px; font-weight: 400">
                  {{ $t("page_course_last_leaning_end") }}
                  {{ stores.last_course.course_code }}</span
                >
              </div>
            </div>
          </div>
          ...
          >
            ...
            >
              <div
                ...
                style="padding: 5px; background-color: #f5f5f5"
              >
                <div style="text-align: center">
                  <span style="font-size: 20px">
                    {{ $t("page_course_congrat") }}</span
                  >
                </div>
        ...
        <div class="row">
          <div class="col-xl-12 col-lg-12 col-md-12 col-12">
            <div class="text-center p-3 mb-3">
              <p class="fs-3">
                {{ $t("already_read") }}: {{ stores.learned }} /
                {{ stores.total_lesson_progress }}
                <span class="text-success">({{ stores.progress }}%)</span>
              </p>
            </div>

            <div
              class="d-flex gap-3 justify-content-center flex-wrap"
              v-if="stores.last_course"
            >
              <button class="btn btn-primary" @click="GotoPage('/course')">
                {{ $t("page_course_last_leaning_go_back") }}
              </button>
              <button
                class="btn"
                style="background-color: var(--dotColor)"
                @click="GotoPage('/exam')"
              >
                {{ $t("page_course_last_leaning_try_course") }}
              </button>
            </div>
          </div>
        </div>
